...
namespace Nitrocoin.Bitcoin.IntegrationTests
{
	...
	public class NitrocoinBitcoinRunner : INodeRunner
	{
	...
	public class NodeBuilder : IDisposable
	{
		public static NodeBuilder Create([CallerMemberNameAttribute] string caller = null, string version = "0.13.1")
		{
			...
			if (retryDelete)
			{
				...
				{
					if (bitcoind.MainModule.FileName.Contains("Nitrocoin.Bitcoin.Tests"))
					{
						bitcoind.Kill();
					}
				}
		...
		public CoreNode CreateNitrocoinNode(bool start = false)
		{
			string child = CreateNewEmptyFolder();
			var node = new CoreNode(child, new NitrocoinBitcoinRunner(), this);
			Nodes.Add(node);
			if (start)
				node.Start();
			return node;
	    ...
	    public CoreNode CloneNitrocoinNode(CoreNode cloneNode)
	    {
	        var node = new CoreNode(cloneNode.Folder, new NitrocoinBitcoinRunner(), this, false);
	        Nodes.Add(node);
	...
	public class CoreNode
	{
		...
		/// <summary>
		/// Get Nitrocoin full node if possible
		/// </summary>
		public FullNode FullNode
		{
			get { return ((NitrocoinBitcoinRunner) _Runner).FullNode; }
		}
		...
		public bool AddToNitrocoinMempool(Transaction trx)
		{
			var fullNode = (_Runner as NitrocoinBitcoinRunner).FullNode;
			var state = new MempoolValidationState(true);

			return fullNode.MempoolManager.Validator.AcceptToMemoryPool(state, trx).Result;
		}

		public Block[] GenerateNitrocoin(int blockCount, List<Transaction> passedTransactions = null, bool broadcast = true)
		{
			var fullNode = (_Runner as NitrocoinBitcoinRunner).FullNode;
