    ...
    public class MemoryPoolTests
	{
		...
		public void AddToMempool()
		{
			using (NodeBuilder builder = NodeBuilder.Create())
			{
				...
				NitrocoinNodeSync.GenerateNitrocoin(105); // coinbase maturity = 100
				...
				var block = NitrocoinNodeSync.FullNode.BlockStoreManager.BlockRepository.GetAsync(NitrocoinNodeSync.FullNode.Chain.GetBlock(4).HashBlock).Result;
				var prevTrx = block.Transactions.First();
				var dest = new BitcoinSecret(new Key(), NitrocoinNodeSync.FullNode.Network);

				Transaction tx = new Transaction();
				tx.AddInput(new TxIn(new OutPoint(prevTrx.GetHash(), 0), PayToPubkeyHashTemplate.Instance.GenerateScriptPubKey(NitrocoinNodeSync.MinerSecret.PubKey)));
				tx.AddOutput(new TxOut("25", dest.PubKey.Hash));
				tx.AddOutput(new TxOut("24", new Key().PubKey.Hash)); // 1 btc fee
				tx.Sign(NitrocoinNodeSync.MinerSecret, false);

				NitrocoinNodeSync.Broadcast(tx);

				TestHelper.WaitLoop(() => NitrocoinNodeSync.CreateRPCClient().GetRawMempool().Length == 1);
			}
		}
		...
		public void AddToMempoolTrxSpendingTwoOutputFromSameTrx()
		{
			using (NodeBuilder builder = NodeBuilder.Create())
			{
				...
				var block = NitrocoinNodeSync.FullNode.BlockStoreManager.BlockRepository.GetAsync(NitrocoinNodeSync.FullNode.Chain.GetBlock(4).HashBlock).Result;
				var prevTrx = block.Transactions.First();
				var dest1 = new BitcoinSecret(new Key(), NitrocoinNodeSync.FullNode.Network);
				var dest2 = new BitcoinSecret(new Key(), NitrocoinNodeSync.FullNode.Network);

				Transaction parentTx = new Transaction();
				parentTx.AddInput(new TxIn(new OutPoint(prevTrx.GetHash(), 0), PayToPubkeyHashTemplate.Instance.GenerateScriptPubKey(NitrocoinNodeSync.MinerSecret.PubKey)));
				parentTx.AddOutput(new TxOut("25", dest1.PubKey.Hash));
				parentTx.AddOutput(new TxOut("24", dest2.PubKey.Hash)); // 1 btc fee
				parentTx.Sign(NitrocoinNodeSync.MinerSecret, false);
				NitrocoinNodeSync.Broadcast(parentTx);
				// wiat for the trx to enter the pool
				TestHelper.WaitLoop(() => NitrocoinNodeSync.CreateRPCClient().GetRawMempool().Length == 1);
				// mine the transactions in the mempool
				NitrocoinNodeSync.GenerateNitrocoin(1, NitrocoinNodeSync.FullNode.MempoolManager.InfoAllAsync().Result.Select(s => s.Trx).ToList());
				TestHelper.WaitLoop(() => NitrocoinNodeSync.CreateRPCClient().GetRawMempool().Length == 0);
				...
				NitrocoinNodeSync.Broadcast(signed);
				TestHelper.WaitLoop(() => NitrocoinNodeSync.CreateRPCClient().GetRawMempool().Length == 1);
			}
		}
		...
		public void MempoolReceiveFromManyNodes()
		{
			using (NodeBuilder builder = NodeBuilder.Create())
			{
				...
				NitrocoinNodeSync.SetDummyMinerSecret(new BitcoinSecret(new Key(), NitrocoinNodeSync.FullNode.Network));
				NitrocoinNodeSync.GenerateNitrocoin(201); // coinbase maturity = 100
				TestHelper.WaitLoop(() => NitrocoinNodeSync.FullNode.ConsensusLoop.Tip.HashBlock == NitrocoinNodeSync.FullNode.Chain.Tip.HashBlock);
				TestHelper.WaitLoop(() => NitrocoinNodeSync.FullNode.ChainBehaviorState.HighestPersistedBlock.HashBlock == NitrocoinNodeSync.FullNode.Chain.Tip.HashBlock);
				...
				{
					var block = NitrocoinNodeSync.FullNode.BlockStoreManager.BlockRepository.GetAsync(NitrocoinNodeSync.FullNode.Chain.GetBlock(index).HashBlock).Result;
					var prevTrx = block.Transactions.First();
					var dest = new BitcoinSecret(new Key(), NitrocoinNodeSync.FullNode.Network);
				...
				{
					NitrocoinNodeSync.Broadcast(transaction);
				});

				TestHelper.WaitLoop(() => NitrocoinNodeSync.CreateRPCClient().GetRawMempool().Length == 100);
			}
		}
		...
		public void TxMempoolBlockDoublespend()
		{
			using (NodeBuilder builder = NodeBuilder.Create())
			{
				...
				builder.StartAll();
				NitrocoinNodeSync.NotInIBD();
				NitrocoinNodeSync.FullNode.Settings.RequireStandard = true; // make sure to test standard tx

				NitrocoinNodeSync.SetDummyMinerSecret(new BitcoinSecret(new Key(), NitrocoinNodeSync.FullNode.Network));
				NitrocoinNodeSync.GenerateNitrocoin(100); // coinbase maturity = 100
				TestHelper.WaitLoop(() => NitrocoinNodeSync.FullNode.ConsensusLoop.Tip.HashBlock == NitrocoinNodeSync.FullNode.Chain.Tip.HashBlock);
				TestHelper.WaitLoop(() => NitrocoinNodeSync.FullNode.ChainBehaviorState.HighestPersistedBlock.HashBlock == NitrocoinNodeSync.FullNode.Chain.Tip.HashBlock);
				...
				var scriptPubKey = PayToPubkeyHashTemplate.Instance.GenerateScriptPubKey(NitrocoinNodeSync.MinerSecret.PubKey);
				var genBlock = NitrocoinNodeSync.FullNode.BlockStoreManager.BlockRepository.GetAsync(NitrocoinNodeSync.FullNode.Chain.GetBlock(1).HashBlock).Result;
				...
				foreach (var index in Enumerable.Range(1, 2))
				{
					...
					// Sign:
					trx.Sign(NitrocoinNodeSync.MinerSecret, false);
					spends.Add(trx);
				}

				// Test 1: block with both of those transactions should be rejected.
				var block = NitrocoinNodeSync.GenerateNitrocoin(1, spends).Single();
				TestHelper.WaitLoop(() => NitrocoinNodeSync.FullNode.ConsensusLoop.Tip.HashBlock == NitrocoinNodeSync.FullNode.Chain.Tip.HashBlock);
				Assert.True(NitrocoinNodeSync.FullNode.Chain.Tip.HashBlock != block.GetHash());

				// Test 2: ... and should be rejected if spend1 is in the memory pool
				Assert.True(NitrocoinNodeSync.AddToNitrocoinMempool(spends[0]));
				block = NitrocoinNodeSync.GenerateNitrocoin(1, spends).Single();
				TestHelper.WaitLoop(() => NitrocoinNodeSync.FullNode.ConsensusLoop.Tip.HashBlock == NitrocoinNodeSync.FullNode.Chain.Tip.HashBlock);
				Assert.True(NitrocoinNodeSync.FullNode.Chain.Tip.HashBlock != block.GetHash());
				NitrocoinNodeSync.FullNode.MempoolManager.Clear().Wait();

				// Test 3: ... and should be rejected if spend2 is in the memory pool
				Assert.True(NitrocoinNodeSync.AddToNitrocoinMempool(spends[1]));
				block = NitrocoinNodeSync.GenerateNitrocoin(1, spends).Single();
				TestHelper.WaitLoop(() => NitrocoinNodeSync.FullNode.ConsensusLoop.Tip.HashBlock == NitrocoinNodeSync.FullNode.Chain.Tip.HashBlock);
				Assert.True(NitrocoinNodeSync.FullNode.Chain.Tip.HashBlock != block.GetHash());
				NitrocoinNodeSync.FullNode.MempoolManager.Clear().Wait();
				...
				Assert.True(NitrocoinNodeSync.AddToNitrocoinMempool(spends[1]));
				block = NitrocoinNodeSync.GenerateNitrocoin(1, oneSpend).Single();
				TestHelper.WaitLoop(() => NitrocoinNodeSync.FullNode.ConsensusLoop.Tip.HashBlock == NitrocoinNodeSync.FullNode.Chain.Tip.HashBlock);
				Assert.True(NitrocoinNodeSync.FullNode.Chain.Tip.HashBlock == block.GetHash());

				// spends[1] should have been removed from the mempool when the
				// block with spends[0] is accepted:
				TestHelper.WaitLoop(() => NitrocoinNodeSync.FullNode.MempoolManager.MempoolSize().Result == 0);
			}
		}
		...
		public void TxMempoolMapOrphans()
		{
			...
			using (NodeBuilder builder = NodeBuilder.Create())
			{
				...
				NitrocoinNode.SetDummyMinerSecret(new BitcoinSecret(new Key(), NitrocoinNode.FullNode.Network));
				...
				for (ulong i = 0; i < 50; i++)
				{
					...
					tx.AddOutput(new TxOut(new Money(1*Money.CENT), NitrocoinNode.MinerSecret.ScriptPubKey));
					
					NitrocoinNode.FullNode.MempoolManager.Orphans.AddOrphanTx(i, tx).Wait();
				}

				Assert.Equal(NitrocoinNode.FullNode.MempoolManager.Orphans.OrphansList().Count, 50);

				// ... and 50 that depend on other orphans:
				for (ulong i = 0; i < 50; i++)
				{
					var txPrev = NitrocoinNode.FullNode.MempoolManager.Orphans.OrphansList().ElementAt(rand.Next(NitrocoinNode.FullNode.MempoolManager.Orphans.OrphansList().Count));

					Transaction tx = new Transaction();
					tx.AddInput(new TxIn(new OutPoint(txPrev.Tx.GetHash(), 0), new Script(OpcodeType.OP_1)));
					tx.AddOutput(new TxOut(new Money((1 + i + 100) * Money.CENT), NitrocoinNode.MinerSecret.ScriptPubKey));
					NitrocoinNode.FullNode.MempoolManager.Orphans.AddOrphanTx(i, tx).Wait();
				}

				Assert.Equal(NitrocoinNode.FullNode.MempoolManager.Orphans.OrphansList().Count, 100);
				...
				for (ulong i = 0; i < 10; i++)
				{
					...
					Assert.False(NitrocoinNode.FullNode.MempoolManager.Orphans.AddOrphanTx(i, tx).Result);
				}
				...
				for (ulong i = 0; i < 3; i++)
				{
					var sizeBefore = NitrocoinNode.FullNode.MempoolManager.Orphans.OrphansList().Count;
					NitrocoinNode.FullNode.MempoolManager.Orphans.EraseOrphansFor(i).Wait();
					Assert.True(NitrocoinNode.FullNode.MempoolManager.Orphans.OrphansList().Count < sizeBefore);
				}

				// Test LimitOrphanTxSize() function:
				NitrocoinNode.FullNode.MempoolManager.Orphans.LimitOrphanTxSize(40).Wait();
				Assert.True(NitrocoinNode.FullNode.MempoolManager.Orphans.OrphansList().Count <= 40);
				NitrocoinNode.FullNode.MempoolManager.Orphans.LimitOrphanTxSize(10).Wait();
				Assert.True(NitrocoinNode.FullNode.MempoolManager.Orphans.OrphansList().Count <= 10);
				NitrocoinNode.FullNode.MempoolManager.Orphans.LimitOrphanTxSize(0).Wait();
				Assert.True(!NitrocoinNode.FullNode.MempoolManager.Orphans.OrphansList().Any());
			}
		}
		...
		public void MempoolAddNodeWithOrphans()
		{
			using (NodeBuilder builder = NodeBuilder.Create())
			{
				...
				NitrocoinNodeSync.SetDummyMinerSecret(new BitcoinSecret(new Key(), NitrocoinNodeSync.FullNode.Network));
				NitrocoinNodeSync.GenerateNitrocoin(101); // coinbase maturity = 100
				TestHelper.WaitLoop(() => NitrocoinNodeSync.FullNode.ConsensusLoop.Tip.HashBlock == NitrocoinNodeSync.FullNode.Chain.Tip.HashBlock);
				...
				var block = NitrocoinNodeSync.FullNode.BlockStoreManager.BlockRepository.GetAsync(NitrocoinNodeSync.FullNode.Chain.GetBlock(1).HashBlock).Result;
				var prevTrx = block.Transactions.First();
				var dest = new BitcoinSecret(new Key(), NitrocoinNodeSync.FullNode.Network);
				...
				txOrphan.Sign(key.GetBitcoinSecret(NitrocoinNodeSync.FullNode.Network), false);

				// broadcast the orphan
				NitrocoinNodeSync.Broadcast(txOrphan);
				TestHelper.WaitLoop(() => NitrocoinNodeSync.FullNode.MempoolManager.Orphans.OrphansList().Count == 1);
				// broadcast the parent
				NitrocoinNodeSync.Broadcast(tx);
				TestHelper.WaitLoop(() => NitrocoinNodeSync.FullNode.MempoolManager.Orphans.OrphansList().Count == 0);
				// wait for orphan to get in the pool
				TestHelper.WaitLoop(() => NitrocoinNodeSync.CreateRPCClient().GetRawMempool().Length == 2);
			}
		}
		...
		public void MempoolSyncTransactions()
		{
			using (NodeBuilder builder = NodeBuilder.Create())
			{
				...
				NitrocoinNode1.FullNode.ChainBehaviorState.SetIsInitialBlockDownload(false, DateTime.UtcNow.AddMinutes(5));
				NitrocoinNode2.FullNode.ChainBehaviorState.SetIsInitialBlockDownload(false, DateTime.UtcNow.AddMinutes(5));
				...
				TestHelper.WaitLoop(() => NitrocoinNodeSync.CreateRPCClient().GetRawMempool().Length == 5);

				// the full node should be connected to both nodes
				Assert.Equal(NitrocoinNodeSync.FullNode.ConnectionManager.ConnectedNodes.Count(), 2);

				// reset the trickle timer on the full node that has the transactions in the pool
				foreach (var node in NitrocoinNodeSync.FullNode.ConnectionManager.ConnectedNodes) node.Behavior<MempoolBehavior>().NextInvSend = 0;

				TestHelper.WaitLoop(() => NitrocoinNode1.CreateRPCClient().GetRawMempool().Length == 5);
				TestHelper.WaitLoop(() => NitrocoinNode2.CreateRPCClient().GetRawMempool().Length == 5);
				...
				TestHelper.WaitLoop(() => NitrocoinNode1.CreateRPCClient().GetBestBlockHash() == NitrocoinNodeSync.CreateRPCClient().GetBestBlockHash()); 
				TestHelper.WaitLoop(() => NitrocoinNode2.CreateRPCClient().GetBestBlockHash() == NitrocoinNodeSync.CreateRPCClient().GetBestBlockHash());
				TestHelper.WaitLoop(() => NitrocoinNode1.CreateRPCClient().GetRawMempool().Length == 0);
				TestHelper.WaitLoop(() => NitrocoinNode2.CreateRPCClient().GetRawMempool().Length == 0);
			}
		}
	}
