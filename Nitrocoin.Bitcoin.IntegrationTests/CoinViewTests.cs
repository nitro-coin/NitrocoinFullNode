	...
	public class CoinViewTests
	{
		...
		public void CanHandleReorgs()
		{
			using(NodeBuilder builder = NodeBuilder.Create())
			{
				...
				//Core1 discovers 10 blocks, sends to Nitrocoin
				var tip = coreNode1.FindBlock(10).Last();
				NitrocoinNode.CreateRPCClient().AddNode(coreNode1.Endpoint, true);
			    TestHelper.WaitLoop(() => NitrocoinNode.CreateRPCClient().GetBestBlockHash() == coreNode1.CreateRPCClient().GetBestBlockHash());
				NitrocoinNode.CreateRPCClient().RemoveNode(coreNode1.Endpoint);

				//Core2 discovers 20 blocks, sends to Nitrocoin
				tip = coreNode2.FindBlock(20).Last();
				NitrocoinNode.CreateRPCClient().AddNode(coreNode2.Endpoint, true);
			    TestHelper.WaitLoop(() => NitrocoinNode.CreateRPCClient().GetBestBlockHash() == coreNode2.CreateRPCClient().GetBestBlockHash());
				NitrocoinNode.CreateRPCClient().RemoveNode(coreNode2.Endpoint);
				((CachedCoinView)NitrocoinNode.FullNode.CoinView).FlushAsync().Wait();

				//Core1 discovers 30 blocks, sends to Nitrocoin
				tip = coreNode1.FindBlock(30).Last();
				NitrocoinNode.CreateRPCClient().AddNode(coreNode1.Endpoint, true);
			    TestHelper.WaitLoop(() => NitrocoinNode.CreateRPCClient().GetBestBlockHash() == coreNode1.CreateRPCClient().GetBestBlockHash());
				NitrocoinNode.CreateRPCClient().RemoveNode(coreNode1.Endpoint);

				//Core2 discovers 50 blocks, sends to Nitrocoin
